...
### PYTHONHOME conflicts with uv python
`PYTHONHOME` is set to the uv python directory (`~/AppData/Roaming/uv/python/...`). When running Python scripts via `python` command, this causes `SRE module mismatch` or `ImportError: cannot import name 'NamespaceLoader'` because the system Python binary loads modules from the uv directory (wrong version). **Fix:** unset `PYTHONHOME` before running scripts:
```bash
unset PYTHONHOME && python your_script.py
```
Or in bat files: `set PYTHONHOME=` before the python command.

### OAuth on Windows (loopback callback issue)

Many Hermes OAuth flows (Codex, Nous Portal, Google OAuth) open a browser
and expect a callback to `http://127.0.0.1:<port>`. On Windows, the
browser often **cannot reach the loopback listener** running inside the
Hermes terminal (especially with Windows Terminal or SSH remotes).

**Workaround:** Use `--no-browser` flag to get a URL to paste manually:
```bash
hermes auth add openai-codex --type oauth --no-browser
```

If `--no-browser` is not available, use `--manual-paste`:
```bash
hermes auth add <provider> --type oauth --manual-paste
```
...
The `image_gen` toolset uses plugins for different providers. Key providers:

| Provider | Auth | Notes |
|----------|------|-------|
| `openai-codex` | OAuth (Codex/ChatGPT) | **No API key needed.** Uses ChatGPT OAuth. |
| `fal` | API key | Set `FAL_KEY` in `.env` |
| `openai` | API key | Set `OPENAI_API_KEY` |

#### gpt-image-2-medium via openai-codex (Codex OAuth)

**Setup:**
```bash
hermes auth add openai-codex --type oauth --no-browser
```
`--no-browser` outputs a URL to paste into a browser — required on Windows since the OAuth callback cannot reach a local loopback listener. After authorization, paste the final callback URL back into the terminal.

**Config:** The model and provider are set in `config.yaml`:
```yaml
image_gen:
...
**Pitfall:** `hermes auth codex` does NOT exist. The correct command is `hermes auth add openai-codex --type oauth [--no-browser]`.

**PowerShell caveat:** Provider names with hyphens (e.g. `openai-codex`) are interpreted as subtraction operators in bare PowerShell arguments. **Always quote the provider name:** `hermes auth add "openai-codex" --type oauth --no-browser`.
...
### Config changes — always use CLI, never edit files directly
**CRITICAL: Never directly edit `config.yaml` to change model/provider/settings.** Use the Hermes CLI commands instead:
```bash
...
```
Why: direct file edits may not take effect (encoding issues, format mismatches, or the gateway caches the old config). CLI commands handle all edge cases and are the only reliable way to change config. User has confirmed that direct edits failed while `hermes config set` worked immediately.
...
**Per-platform model switching is NOT supported.** All messaging platforms (Feishu, Telegram, Discord, etc.) share the same agent and the same model configuration (`model.default` in config.yaml). There is no mechanism to route Feishu users to a different model than Telegram users — the gateway has a single model config that applies uniformly.
...
When troubleshooting a platform that previously showed errors, check the **most recent** gateway log entries — not mid-session historical entries. A platform may have errored earlier (e.g. first startup failure) but recovered successfully on a later restart. Reporting an old error as current when the platform is already healthy produces false alarms.

**Correct sequence:**
1. `tail -20 ~/.hermes/logs/gateway.log` — most recent state
2. Confirm the error is still occurring (not just historical)
3. Then report
...
| Config write protection | On Windows, direct `patch` on `config.yaml` fails — use `hermes config set` instead. See `references/config-yaml-write-protection.md` |
