...
### 🔴🔴🔴 更新日志"声称修复"但表格未更新（2026-07-19发现，反复出现）

**问题**：更新日志条目写"修复V20评论/分享互换（评论0→3，分享3→0）"，但**表格行本身没有被修改**。下次审核时发现表格仍然是旧值。

**根因**：agent把"写更新日志条目"等同于"完成修复"，忘记实际用patch修改表格行。

**案例（2026-07-18 11:00）**：
- 更新日志写："修复V20评论/分享互换（评论0→3，分享3→0）"
- 但表格行仍然是：`| V20 | ... | 9 | 0 | 3 | 3 |`（评论0，分享3）
- 实际页面数据：评论3，分享0
- → 表格从未被修改，"修复"只是写了条日志

**强制执行规则**：
1. **"修复"必须同时修改两处**：①表格行（用patch）②更新日志条目（用echo/append）
2. **只写更新日志不改表格 = 未完成修复**
3. **修复后必须用grep验证表格行已变化**：
   ```bash
   grep "V20" 发展日志.md | head -1
   # 确认输出中评论=3，分享=0
   ```
...
### ⚠️ 无新视频时的处理（2026-07-26验证）

当所有视频都已标记"已完成"且无新发布时，cron job仍应：
1. 提取投稿列表数据，与发展日志对比
2. 检查是否存在评论/分享互换错误（用 `cells[9]=分享, cells[10]=评论` 验证）
3. 更新微量数据变化（5s完播率、2s跳出率的微小波动）
4. 在发展日志末尾追加时间戳条目，记录本次检查结果
5. **追加条目用 `head -N > /tmp && echo >> /tmp && cp` 模式**（不要用 `cat >>`，会丢失同session已patch的内容）
...
⚠️ 优化分组表计数验证（必须用Python批量计算，2026-07-25验证）：
优化分组表中的"XX条视频"计数（如"22条完播率<10%"）在新视频添加后会过时。
不要信任表中声称的计数，用Python脚本遍历全部视频重新计算：

python -c "
videos = {'V28': {'plays': 368, 'comments': 0, ...}, ...}  # 从表格提取
# 评论率<0.3%计数
cr_count = sum(1 for v in videos.values() if v['plays'] > 0 and v['comments']/v['plays']*100 < 0.3)
print(f'comment_rate<0.3%: {cr_count} videos')
# 点赞率<1%计数
lr_count = sum(1 for v in videos.values() if v['plays'] > 0 and v['likes']/v['plays']*100 < 1.0)
print(f'like_rate<1%: {lr_count} videos')
# 完播率<10%计数
comp_count = sum(1 for v in videos.values() if v['completion'] < 10)
print(f'completion<10%: {comp_count} videos')
# 2s跳出率>30%计数
s2_count = sum(1 for v in videos.values() if v['s2'] > 30)
print(f'bounce>30%: {s2_count} videos')
"
对比计算结果与优化分组表声称的计数，不一致则标记修正。

⚠️ 私密/仅自己可见状态检查（2026-07-25发现）：
如果视频被设为"仅自己可见"，其推荐页占比会极低（如V28仅2.2%）、搜索占比极高（如94.2%）。
这些指标受私密状态影响，不能作为内容质量的诊断依据。审核时必须：
1. 检查是否有视频处于私密状态（从里程碑或更新日志中查找"仅自己可见"标记）
2. 如果有，在诊断结论中注明"该视频为私密状态，推荐页/搜索数据不具参考价值"
3. 表格状态应从"已完成"改为"仅自己可见/私密"（如未更新则标记）
```
