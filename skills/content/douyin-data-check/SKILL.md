...
**⚠️ 虚拟滚动限制（2026-07-08更新）**：`document.querySelectorAll('table tbody tr').length` 通常返回11-20（10-19条视频+空行），取决于页面版本和滚动位置。2026-07-08实测返回20行（19条视频+1空行），比之前的10-12行有所改善。对于cron job（只关注新视频+近期变化），10-20行数据已足够。
