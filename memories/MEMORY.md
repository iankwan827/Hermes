Python 3.11坏了。解决：uv venv + `PYTHONHOME=""`。boss-agent-cli：`cd /d/hermes-agent && PYTHONHOME="" .venv-boss/Scripts/boss.exe`。TikHub API key：skill tikhub-api的`references/api_key.txt`。搜索接口`fetch_general_search_v2`不带粉丝数，`fetch_video_search_v1`带粉丝数。低粉爆款库：`D:/hermes-agent/文案/国学命理爆款库.json`。用户探索国学/命理职业方向。
§
...
§
OpenCLI：先`opencli daemon status`，断开就restart。给命令不加bash/zsh，直接给。用户多次纠正过度解释。
§
