...
> 自动生成 by Hermes Cron Job — 2026-07-23
...
### 踩坑记录
- [24课笔记质量] 用户反馈24课笔记内容和课件图片不一致（"地支相破专项"标题和内容对不上，tesseract OCR乱码严重）→ 用小米API重新OCR课件图片（16个切片+8张截屏），再按course-notes-fusion skill的7-Agent流程重新整理笔记（来源：session 20260723_101503）
- [录音课次搞错] 录音启动时判断课次出错，创建了第二十六课文件夹，实际应录第二十五课。用户原话"你刚刚卡了，是25课，是cli创建了25课文件夹" → 杀掉错误进程，删除空文件夹，重新启动正确课次。**教训：录音前必须先确认最新课次文件夹**（来源：session 20260723_101503）

### 新发现
- [八字语录] Day99-99 新增9条语录：比劫食伤（夸人策略）、印星旺（新陈代谢慢显年轻）、印星厚积薄发、官杀重无印闲不下来、大伤官和老板拍桌子、日支偏印守口如瓶、丁火忽冷忽热、钱财有定数、正印精神洁癖、官印领导口才跟不上大脑（来源：session 20260723_101503）
- [第二十四课转录] 转录完成，35,482字，耗时46分钟，使用gemma模型GPU加速（来源：session 20260722_101701）
- [第二十五课录音] 晚间课程已启动录音，PID 43129（来源：session 20260723_200529）

### 用户偏好更新
- **录音课次必须准确**：启动录音前必须先检查文件夹列表确认最新课次，不要凭记忆判断。错误启动后要立即杀掉并重启正确课次

### Skill 更新
- bazi-yulu: 追加 Day99-99 语录（9条），涵盖比劫食伤、印星、官杀、伤官、日支偏印、丁火、正印、官印等多个十神的应用场景
...
- tesseract 中文 OCR 质量极差，课件长图必须逐片用 vision_analyze 或小米API看完
...
### 录音管理
- 启动录音前必须先检查文件夹列表确认最新课次，不要凭记忆判断
- 用户说「开始上课了」后直接执行，不要追问确认
- 录音进程用 `ps aux | grep record.py | grep -v grep` 检查状态
...
- 用户说笔记/课件不对时直接改，不要质疑（"喊你干就干，说你错你就错"）
- 被指出一个错误后必须核查同类内容所有地方，不能只改被指出的那一个
